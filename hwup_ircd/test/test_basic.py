...
class Test_IRC_command(unittest.TestCase):
    ...
    def test_OK1(self): # test su una simulazione di traffico corretto client-server
        traffic = [ (irc_command.command_pass, 'pass provapwd', 'OK'), 
                    ...
                    (irc_command.command_user, 'user guest 0 * :Nome Cognome', 'OK, registered'), 
        ...
        irc_command.command_privmsg(self.client, commandSplit, self.server) # se privmsg da errore manderà una reply al client che andrà finire nell'attributo del mock: reply_output
